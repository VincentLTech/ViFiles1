...
module.exports = {
    ...
    logout: (req, res) => {
        res.clearCookie('userToken').clearCookie('userId').json({message:'User is logged out'})
        // you have to nest it to amke it work
        // res.clearCookie('userToken').json({message:'User is logged out'})
        // res.clearCookie('userId').json({message:'User is logged out'})
